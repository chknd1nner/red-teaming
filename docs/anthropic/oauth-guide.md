...
> **✅ Verified Working**: This approach has been tested and confirmed working as of **December 6th, 2025**. All requirements documented here are based on empirical testing against Anthropic's production API.
...
> **⚠️ CRITICAL: Easy-to-Miss Requirements**
>
> The following requirements are **not standard OAuth** and will cause silent failures if omitted:
>
> 1. **`code=true` parameter** - Must be included in the authorization URL. Without it, clicking "Authorize" completes but the redirect to display the code never happens.
>
> 2. **32 bytes for state** - Use `crypto.randomBytes(32)`, not 16 bytes. Anthropic may have an undocumented minimum entropy requirement.
>
> 3. **State in token exchange** - The `state` parameter must be included in the token exchange request body (not just the authorization URL).
>
> These were discovered through debugging against the working OpenCode CLI implementation (December 2025).
...
### Nothing Happens After Clicking "Authorize"

**Symptom**: User clicks "Authorize" on claude.ai, the authorization appears to complete, but no redirect happens and no code is displayed.

**Cause**: Missing or incorrect parameters in the authorization URL.

**Solutions** (check all three):

1. **Missing `code=true` parameter**
   ```javascript
   // ❌ Wrong - no code parameter
   authUrl.searchParams.set('client_id', CLIENT_ID);

   // ✅ Correct - include code=true
   authUrl.searchParams.set('code', 'true');
   authUrl.searchParams.set('client_id', CLIENT_ID);
   ```

2. **State parameter too short**
   ```javascript
   // ❌ Wrong - only 16 bytes
   const state = crypto.randomBytes(16).toString('base64url');

   // ✅ Correct - use 32 bytes
   const state = crypto.randomBytes(32).toString('base64url');
   ```

3. **State missing from token exchange** (will cause next step to fail)
   ```javascript
   // ❌ Wrong - state not included
   const payload = {
     code: code,
     grant_type: 'authorization_code',
     // ...
